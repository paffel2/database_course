...
class DismissalOrderForm(forms.ModelForm):
    employee = forms.ModelChoiceField(
        queryset=Employee.objects.filter(is_active=True), label="Сотрудник"
    )
...
class SickLeaveOrderForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if start and end and start > end:
            raise forms.ValidationError(
                "Дата начала больничного должна быть раньше даты окончания"
            )
...
class HireOrderForm(forms.ModelForm):
    ...
    position = forms.ModelChoiceField(
        queryset=Position.objects.all(), label="Должность"
    )
    department = forms.ModelChoiceField(
        queryset=Department.objects.all(), label="Отдел"
    )
...
class UpdateVacationOrderForm(forms.ModelForm):
    class Meta:
        model = Vacation
        fields = ["start_date", "end_date", "vacation_type", "is_paid", "employee"]
        widgets = {
            "start_date": forms.DateInput(attrs={"type": "date"}),
            "end_date": forms.DateInput(attrs={"type": "date"}),
        }
...
class UpdateSickLeaveOrderForm(forms.ModelForm):
    class Meta:
        model = SickLeave
        fields = ["start_date", "end_date", "sick_number", "employee"]
        widgets = {
            "start_date": forms.DateInput(attrs={"type": "date"}),
            "end_date": forms.DateInput(attrs={"type": "date"}),
        }
